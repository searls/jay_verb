...
module Japanese
  module VerbIdentifier
    ...
    def resolve_verb_class
      ...
      if kanji == "行く"
        ...
        case kanji[-1]
          ...
          when "く"
            self.part_of_speech = "v5k" unless kanji == "行く"
          when "む"
          ...
          when "う"
            self.part_of_speech = "v5u" unless kanji == "問う"
        end
      end
    end
    ...
    def ambiguous?
      kanji == hiragana && kanji.in?(AMBIGUITY_IF_HIRAGANA_ONLY) ? true : false
    end

    def ends_in_iru_eru?
      hiragana[-2].in?(E_HIRAGANA) || hiragana[-2].in?(I_HIRAGANA) ? true : false
    end

    def is_consonant_verb?
      if ends_in_iru_eru? && !ambiguous?
        kanji.in?(CONSONANT_VERBS) ? true : false
      end
    end
